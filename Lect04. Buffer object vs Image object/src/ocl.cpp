...
OCL::OCL()
{
    ...
    cmdQueue = cl::CommandQueue(context, devices[0], CL_QUEUE_PROFILING_ENABLE, &err); // device 0 is default
    if (err < CL_SUCCESS /*=0*/)
    {
...
OCL::OCL(int platform_num, const char *device_type, int device_num)
{
    ...
    cmdQueue = cl::CommandQueue(context, devices[device_num], CL_QUEUE_PROFILING_ENABLE, &err); // device 0 is default
    if (err < CL_SUCCESS /*=0*/)
    {
        std::cerr << "[ERROR]clCreateCommandQueue error code: " << err << "\n";
        return;
